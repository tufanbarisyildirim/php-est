<?php
	...
	class EST 
	{
		...
		private function __get_credentials() 
		{
			...
			if($this->slug
				&& array_key_exists($this->slug, self::$banksDetails)) 
		...
		public function pay($credit_card_number, $cvv, $month, $year, $amount, $installment, $orderid, $typ = "Auth", $extra = array()) 
		{
			...
			$postData = urlencode("DATA=" . $documentString);
			$this->raw_response = EST::curl($url,'POST',$postData);
		...
		public static function curl($url,$method = 'POST',$data)
		{
			$curl = curl_init();
			// Set the url..
